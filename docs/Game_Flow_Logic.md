...
3.  **白天 (Day Phase)**
    ...
    *   **判决 (Verdict)**:
        ...
        *   触发“被处决时”能力（如：圣徒 Saint、孟德拉 Minstrel）。
    *   **黄昏 (Dusk)**: 检查游戏是否结束。
...
**首夜 (First Night)**:
1.  **Philosopher** (哲学家 - *如果混入此板子*): 优先选择能力。
...
6.  **Undertaker** (送葬者): 获得昨天被处决者的角色。
7.  **Empath/Fortune Teller/Butler**: 同首夜。
8.  **Spy**: 查看魔典。
...
1.  **Philosopher** (哲学家): 获得能力（如果此时发动）。
2.  **Snake Charmer** (舞蛇人): 选择玩家 -> 若是恶魔，互换身份。
    *   *逻辑*: 极其复杂。如果选中恶魔，Snake Charmer 变成新 Demon，原 Demon 变成 Snake Charmer 并中毒。
    *   *注意*: 互换后，新恶魔（原舞蛇人）**当晚立即行动**（因为恶魔还没动过）。
3.  **Witch** (女巫): 诅咒一人。
4.  **Cerenovus** (洗脑师): 指定一人 + 角色 -> 陷入疯狂。
5.  **Pit-Hag** (麻脸巫婆): 选一人 -> 变角色。
    *   *逻辑*: 若变成恶魔，当晚死亡判定（通常死原恶魔）。
6.  **Demon (恶魔)**:
    *   **Fang Gu** (方古): 杀人。若杀外来者 -> 外来者变邪恶方古，原方古死。
    *   **Vigormortis** (亡骨魔): 杀人。若杀爪牙 -> 爪牙保留能力并毒邻居。
    *   **No Dashii** (诺达虱): 杀人。被动：两旁镇民中毒。
    *   **Vortox** (涡流): 杀人。被动：所有信息必须是假的。
7.  **Dreamer** (筑梦师): 查验（一真一假）。
8.  **Flower Girl** (卖花女): 查验恶魔是否投票。
9.  **Oracle** (神谕者): 查验死人中的邪恶数。
...
*   **信息强制为假 (Vortox)**:
    *   若 Vortox 在场，`generate_info()` 函数必须**反转**所有逻辑。
